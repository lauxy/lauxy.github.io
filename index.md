---
layout: academic
title: Xueyuan Liu
position: Senior System Software Engineer
affiliation: Li Auto Inc.
show-avatar: false
css:
  - "/assets/css/academic.css"
---

## About

I received my Ph.D. degree from [ACALab](http://acalab.sjtu.edu.cn), Shanghai Jiao Tong University (SJTU), advised by Prof. Xiaoyao Liang. My research focuses on computer architecture, software-hardware co-design, AI acceleration, and GPGPU.

## Research Interests

- Computer Architecture & Software-Hardware Co-design
- AI Acceleration (Point Cloud Neural Networks)
- General-Purpose Graphics Processing Units (GPGPU)

## News

<ul class="news-list">
<li><span class="news-date">[2025.01]</span> Paper "SAGA" accepted by DAC 2025 (CCF-A).</li>
<li><span class="news-date">[2024.03]</span> Our paper "FusionArch" received <strong>Best Paper Award</strong> at DATE 2024.</li>
<li><span class="news-date">[2024.02]</span> Paper "MoC" accepted by DAC 2024.</li>
<li><span class="news-date">[2023.11]</span> Papers "FusionArch" and "Sava" accepted by DATE 2024.</li>
</ul>

## Publications

### 2025

<div class="pub-item">
Ruiyang Chen, <strong>Xueyuan Liu</strong>, Chunyu Qi, Yuanzheng Yao, Yanan Sun, Xiaoyao Liang, and Zhuoran Song. "SAGA: A Memory-Efficient Accelerator for GANN Construction via Harnessing Vertex Similarity." <span class="pub-venue">Design Automation Conference (DAC 2025, CCF-A).</span>
</div>

### 2024

<div class="pub-item">
<strong>Xueyuan Liu</strong>, Zhuoran Song, Hao Chen, Xing Li, and Xiaoyao Liang. "MoC: A Morton-Code-Based Fine-Grained Quantization for Accelerating Point Cloud Neural Networks." <span class="pub-venue">Design Automation Conference (DAC 2024).</span> <span class="pub-links">[<a href="/2024-02-14-MoC_ A Morton-Code-Based Fine-Grained Quantization for Accelerating Point Cloud Neural Networks/">blog</a>]</span>
</div>

<div class="pub-item">
<strong>Xueyuan Liu</strong>, Zhuoran Song, Guohao Dai, Gang Li, Can Xiao, Yan Xiang, Dehui Kong, Ke Xu and Xiaoyao Liang. "FusionArch: A Fusion-Based Accelerator for Point-Based Point Cloud Neural Networks." <span class="pub-venue">Design, Automation, and Test in Europe (DATE 2024).</span> <span class="pub-award">Best Paper Award.</span> <span class="pub-links">[<a href="/2023-11-12-FusionArch_ A Fusion-Based Accelerator for Point-Based Point Cloud Neural Networks/">blog</a>]</span>
</div>

<div class="pub-item">
<strong>Xueyuan Liu</strong>, Zhuoran Song, Xiang Liao, Xing Li, Tao Yang, Fangxin Liu and Xiaoyao Liang. "Sava: A Spatial- and Value-Aware Accelerator for Point Cloud Transformers." <span class="pub-venue">Design, Automation, and Test in Europe (DATE 2024).</span> <span class="pub-links">[<a href="/2023-11-12-Sava_ A Spatial- and Value-Aware Accelerator for Point Cloud Transformer/">blog</a>]</span>
</div>

<div class="pub-item">
Xing Li, Zhuoran Song, Rachata Ausavarungnirun, Xiao Liu, <strong>Xueyuan Liu</strong>, Xuan Zhang, Xuhang Wang, Jiayao Ling, Gang Li, Naifeng Jing, Xiaoyao Liang. "Janus: A Flexible Processing-in-Memory Graph Accelerator Towards Sparsity." <span class="pub-venue">IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems, 2024.</span> <span class="pub-links">[<a href="https://ieeexplore.ieee.org/abstract/document/10540086" target="_blank">paper</a>]</span>
</div>

<div class="pub-item">
Xuan Zhang, Zhuoran Song, Peng Zhou, Xing Li, <strong>Xueyuan Liu</strong>, Xiaolong Lin, Zhezhi He, Li Jiang, Naifeng Jing and Xiaoyao Liang. "WIFA: A Weight Importance- and Frequency-aware Accelerator for SNN." <span class="pub-venue">IEEE International Conference on Computer Design (ICCD 2024, CCF-B).</span>
</div>

### 2022

<div class="pub-item">
Xing Li, Rachata Ausavarungnirun, Xiao Liu, <strong>Xueyuan Liu</strong>, Xuan Zhang, Heng Lu, Zhuoran Song, Naifeng Jing and Xiaoyao Liang. "Gzippo: Highly-compact Processing-In-Memory Graph Accelerator Alleviating Sparsity and Redundancy." <span class="pub-venue">IEEE/ACM International Conference on Computer-Aided Design (ICCAD 2022).</span> <span class="pub-links">[<a href="https://rausavar.github.io/pubs/li_iccad22.pdf" target="_blank">paper</a>]</span>
</div>

<div class="pub-item">
Contributed to the writing of "General-Purpose Graphics Processing Units Design — GPGPU Programming Model and Architecture Principles" (通用图形处理器设计——GPGPU编程模型和架构原理). <span class="pub-venue">Tsinghua University Press, 2022.</span> <span class="pub-links">[<a href="http://www.tup.tsinghua.edu.cn/booksCenter/book_09459601.html" target="_blank">view</a>]</span>
</div>

### 2020

<div class="pub-item">
Zhuoran Song, Feiyang Wu, <strong>Xueyuan Liu</strong>, Naifeng Jing and Xiaoyao Liang. "VR-DANN: Real-Time Video Recognition via Decoder-Assisted Neural Network Acceleration." <span class="pub-venue">IEEE/ACM International Symposium on Microarchitecture (MICRO 2020).</span> <span class="pub-links">[<a href="https://ieeexplore.ieee.org/abstract/document/9251974/" target="_blank">paper</a>]</span>
</div>

## Education

<div class="edu-item">
<strong>Ph.D. in Computer Science and Technology</strong>
Shanghai Jiao Tong University <span class="edu-time">| Sep. 2020 – Jun. 2025</span>
</div>

<div class="edu-item">
<strong>B.Eng. in Computer Science and Technology</strong>
Northeastern University <span class="edu-time">| Sep. 2016 – Jun. 2020</span>
</div>
